...
def detect(save_img=False):
    ...
    if classify:
        ...
        modelc.load_state_dict(torch.load('weights/resnet50.pt', map_location=device)['model']).to(device).eval()
                    ...
                    if save_img or view_img:  # Add bbox to image
                        ...
                        if opt.heads or opt.person:
                            if 'head' in label and opt.heads:
                                plot_one_box(xyxy, im0, label=label, color=[int(cls)], line_thickness=3)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='bodyhead.pt', help='model.pt path(s)')
    ...
    parser.add_argument('--project', default=None, help='save results to project/name')
